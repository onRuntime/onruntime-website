...
| `defaults` | `SitemapDefaults` | Não | Valores padrões para todas as entradas |
| `locales` | `string[]` | Não | Idiomas suportados |
| `defaultLocale` | `string` | Não | Idioma padrão |
...
|--------|------|----------|-------------|
| `baseUrl` | `string` | Sim | A URL base do seu site |
| `rules` | `RobotsRule[]` | Não | Regras de robôs personalizadas |
...
| Property | Type | Description |
|----------|------|-------------|
| `userAgent` | `string` | Agente usuário alvo |
...
<Callout type="info" title="Suporte ao TypeScript">
